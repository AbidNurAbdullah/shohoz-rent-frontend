      ...
      <div className="pt-28 pb-10 px-4 flex flex-col xl:flex-row gap-10 w-full max-w-7xl mx-auto">
        ...
        <div className="w-full xl:w-2/3 rounded-xl p-6 shadow-xl bg-black/10 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-6 mb-8 bg-white/20 p-4 rounded-lg">
            <img src={currentUser?.avatar ? `http://localhost:8800${currentUser.avatar}` : "/default.png"} alt="avatar" className="w-20 h-20 rounded-full border-2 border-white object-cover" />
                ...
                <div key={post.id} className="relative group">
                  <Link to={`/${post.id}`} className="block">
                    <div className="bg-white/30 backdrop-blur-md rounded-xl p-4 flex gap-4 shadow-md hover:bg-white/40 transition">
                      <img src={post.images[0] ? `http://localhost:8800${post.images[0]}` : "/apartment2.jpg"} className="w-40 h-28 rounded-lg object-cover" alt="" />
                  ...
                  <div className="absolute top-4 right-4 flex gap-2">
                    <Link 
                      to={`/edit/${post.id}`} 
                      className="bg-gray-900 hover:text-gray-900 hover:bg-white text-white px-3 py-1 rounded text-xs transition font-semibold"
                    >
                      Edit
                    </Link>
                    <button 
                      onClick={(e) => handleDeletePost(e, post.id)} 
                      className="bg-red-600/80 hover:bg-red-600 text-white px-3 py-1 rounded text-xs transition font-semibold"
                    >
                      Delete
                    </button>
                  </div>
              ...
              savedPosts.map((post) => (
                <Link to={`/${post.id}`} key={post.id} className="block">
                  <div className="bg-white/20 p-4 rounded-xl flex gap-4 hover:bg-white/30 transition shadow-md">
                    <img src={post.images[0] ? `http://localhost:8800${post.images[0]}` : "/apartment2.jpg"} className="w-20 h-16 rounded-md object-cover" alt="" />
